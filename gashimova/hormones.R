...
summary.count.first <- lapply(d.first[3:7], function(x) paste(round(mean(x, na.rm=T), 4), "±", round(sd(x, na.rm=T)/sqrt(length(x[is.na(x)==F])), 4), sep=''))
summary.count.second <- lapply(d.second[3:7], function(x) paste(round(mean(x, na.rm=T), 4), "±", round(sd(x, na.rm=T)/sqrt(length(x[is.na(x)==F])), 4), sep=''))
